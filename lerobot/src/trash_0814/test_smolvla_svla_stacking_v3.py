#!/usr/bin/env python3
"""
SmolVLA 모델에 svla_so100_stacking 데이터 1 episode를 넣어서 결과값을 출력하는 코드 (v3)
...
def create_test_config():
    ...
    train_config = TrainPipelineConfig(
        ...
        batch_size=1,
        steps=1,  # 테스트용으로 1 step만
        eval_freq=0,  # 평가 비활성화
        log_freq=1,
        save_checkpoint=False,
        output_dir=Path("outputs/test_smolvla")
    )
...
def test_smolvla_inference():
    """SmolVLA 모델 추론 테스트"""
    print("=== SmolVLA svla_so100_stacking 테스트 시작 (v3) ===\n")
    
    # 1. 설정 생성
    print("1. 설정 생성 중...")
    train_config = create_test_config()
